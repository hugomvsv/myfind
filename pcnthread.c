...
int main() {
    ...
    pthread_create(&th_data_array_prod[0]->pthread_id, NULL, &produtor, &th_data_array_prod[0]);


    for (int i = 0; i < N; i++)
        pthread_create(&th_data_array_cons[i]->pthread_id, NULL, &consumidor, &th_data_array_prod[0]);
...
void *produtor(void *param) {
    ...
    semaphore_wait(semPodeProd);        // espera que o semaforo baixe para poder produzir
    ...
    if ((dir = opendir(fullpath)) == NULL) //tenta abrir o diretório passado
    ...
    semaphore_signal(semPodeCons);        // aumenta o semaforo de produtores
    return 0;
}
...
void *consumidor(void *param) {
    ...
    while (1) {
        ...
        consptr = (consptr + 1) %
                  N;        // coloca o consptr para a proxima posicao, se for a ultima posicap passa para a primeira devido ao %N
    ...
    if ((dir = opendir(fullpath)) == NULL) //tenta abrir o diretório passado
        ...
        while ((entry = readdir(dir)) != NULL) //começa a ler todos os dirs dentro do  root passado
        {
            ...
            if (cont ==
                ...
                t_data->nresultados++;
                realpath(entry->d_name, pathsol);    //recolhemos para pathsol o path do match
                strcpy(t_data->pathsol[t_data->nresultados - 1],
                       pathsol); //copiamos para a nossa struct para no final ser apresentado devidamente
            }
        }
        closedir(dir);
    }
